...
// Every task is its own container (see start() above) — with no daemon reachable,
// nothing this app does can work. Never rejects; a timed-out/refused socket just
// means "down". Cheap call (no container list), safe to poll on every request.
function ping() {
  return new Promise((resolve) => {
    execFile('docker', ['version', '--format', '{{.Server.Version}}'], { timeout: 3000 }, (err) => resolve(!err));
  });
}

module.exports = { start, stop, remove, restart, logs, inspect, listNetworks, containerName, dockerizeUrl, ping };
